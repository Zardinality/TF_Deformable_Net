...
def im_detect(sess, net, im, boxes=None):
    ...
    if cfg.TEST.HAS_RPN:
        ...
        feed_dict={net.data: blobs['data'], net.rois: blobs['rois'], net.keep_prob: 0.5}
...
def test_net(sess, net, imdb, weights_filename , max_per_image=300, thresh=0.05, vis=False):
    ...
    for i in range(num_images):
        ...
        for j in range(1, imdb.num_classes):
            ...
            # cls_dets = np.hstack((boxes[:, j*4:(j+1)*4], np.expand_dims(scores[:, j], -1))) \
            #       .astype(np.float32, copy=False)
            # keep = nms(cls_dets, 0.3)
            # cls_scores = scores[keep, j]
            # # print(cls_scores)
            # # cls_boxes = boxes[keep, j*4:(j+1)*4]
            # cls_dets = cls_dets[keep, :]
            # inds = np.where(cls_scores[:] > 0.1)[0]
            # cls_dets = cls_dets[inds, :]
            # print(cls_dets)
            if vis:
                vis_detections(image, imdb.classes[j], cls_dets, thresh)
...
def load_test_net(sess, net, imdb, weights_filename , max_per_image=300, thresh=0.05, vis=False):
    output_dir = get_output_dir(imdb, weights_filename)
    det_file = os.path.join(output_dir, 'detections.pkl')
    with open(det_file, 'rb') as f:
